...
def loc2bary(mjd, dm, ref_freq, location, source, dm_const=1/0.241):
...
def options():
    ...
    general.add_argument('-r', '--half_range', type=float, default=2.0,
...
# load db pickle file
#  - get components + ACF-ranges
#  - use half ACF range as first guess for Gauss fits
#  - get peak times for guess of MJD
# load archive pazi file
#  - create flagging mask
#  - from archive T0 and central peak time get approximate MJD of bursts
#  - use that (+/- 1s) to cutout relevant range in filterbank
# load filterbank, pass flagging mask, t_burst +/- 1s, dm=0 for sfxc, dm=410.775 for stockert.
#  - integrate in frequency
#  - show time series, user has to pick the desired range; both the on-pulse and the off-pulse region
#  - using this
#
...
if __name__ == "__main__":
    args = options()
    db = pd.read_csv(args.db, sep=",", header=0, index_col=0, na_values='NA')
    ...
    #arr, on_range, off_range = main(db=db, ID=ID, fil=fil, src=src, archive=archive,
    #                                burst_time=burst_time, tscrunch=tscrunch)
    ...
    print(secs_in, burst_time, T0_fil)
    ...
    try:
        ...
        half_range = dur / 2
        secs_in = half_range
        print(f"secs_in after negative: {secs_in}")
    ...
    if flags is None:
        flags = np.ones_like(arr, dtype=bool)
    ...
    arr = get_clean_dyn(arr,arr[:, off_range[0]:off_range[1]])
    #print(arr.shape)

    profile = arr.mean(axis=0)
    profile_full = convert_SN(profile, profile[off_range[0]:off_range[1]])
    print(profile_full.shape)
    #profile_fill = profile_full 
    ...
    if show_dynspec:
        ...
        #def fit_profile(profile, twidths, tsamp, npeaks, on_range=None):
    ...
    db.to_csv(args.db, sep=",", header=True, index=True, na_rep='NA')
