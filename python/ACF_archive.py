...
#from burst_2d_Gaussian import drifting_2DGaussian
...
def outdir_plots(archive_name, expname):
    "create outdir for plots in the git repo"

    file_name = archive_name.split("/")[6]

    #Location of the python script
    script_dir = os.path.dirname(os.path.realpath(__file__))

    #Move up one folder in the path
    parent_dir = os.path.abspath(os.path.join(script_dir, os.pardir))

    #Saving location of script
    out_dir = parent_dir + "/plots/diag_plots" + f"/{expname}"
    create_dir(out_dir)
    print(f"made {out_dir} for saving")

    #outdir + filename, only need to add the ACF or 2DACF to the filename for the save location
    pre_extension = out_dir + "/" + file_name
...
def options():
    ...
    parser.add_argument('--id', type=str, required=True,
                        help='Adding the ID of the burst to the dataframe, no capitals, ex: B01-wb')
...
if __name__ == "__main__":
    ...
    if args.db is not None:
        ...
        if not os.path.exists(args.db):
            ...
            data = pd.read_csv(args.db, sep=",", header=0, index_col=0, na_values='NA')
            ncolumns = data.columns.shape[0]
        try:
            exp, dish, scan = (os.path.basename(args.arfile)).split('_')[0:3]
            #Due to this hardcoding of expected name, which works for EVN dishes I have to adjust the dish part here to "st"
            if exp=='st_mkt':
                print(f"FOUND STOCKERT: Changing the telescope to stockert (st) for the pickle file")
                dish='st'
        except:
        ...
        # Create outdit for diag plots, using the full args name and the exp name
        pre_extension = outdir_plots(args.arfile, exp)
            ...
            if not df_comp.empty:
                ...
                for component in range(len(central_bins)):
                    ...
                    data.loc[(data.experiment == exp) & 
                            ...
                            (data.src == src) &
                            (data.scan == scan) & 
                            (data.component == component),'id'] = args.id
            else:
        ...
        if args.newfit:
            ...
            subbursts = identify_bursts(np.mean(archive, axis=0),
                                        f"Identify edges and peaks of subcomponents for the ACF.\n tsamp:{tsamp} s | fsamp:{fsamp} MHz")
            ranges = np.array(subbursts.ranges).reshape(-1, 2)
            central_bins = np.array(subbursts.peak_times)
            subbursts = identify_bursts(np.mean(archive, axis=0),
                                        f"Please choose the off-pulse region (in time).\n tsamp:{tsamp} s | fsamp:{fsamp} MHz")
            off_range_t = np.array(subbursts.ranges, dtype=int)
    ...
    for component, Range in enumerate(ranges):
        ...
        fit_mask = fit_mask.astype(np.float32)
        ...
        if args.oned is not None:
            ...
            if args.oned  == 'f':
                ...
                #ACF_for_fit_2 = ACFmasked[int(len(ACF)/2.-(20/freq_res)):int(len(ACF)/2.+(20/freq_res))]
                #x_for_fit_2 = freqs[int(len(ACF)/2.-(20/freq_res)):int(len(ACF)/2.+(20/freq_res))]
                #print("*** Lorentzian fit to freq ACF ***")
                #result2 = gmodel.fit(ACF_for_fit_2, x=x_for_fit_2, gamma=5, y0=1, c=0)
            if args.oned == 't':
            ...
            #ax[0].plot(x_for_fit_2, ACF_for_fit_2,'k',alpha=0.3)
            #ax[0].plot(x_for_fit_2,lorentz(x_for_fit_2,result.params['gamma'],result.params['y0'],result.params['c']),color='orange')
            #ax[0].set_xlim(-0.01,0.01)
            #ax[1].plot(x_for_fit_2, ACF_for_fit_2,'k',alpha=0.3)
            #ax[1].
            plt.plot(x, ACFmasked,'k',alpha=0.8)
            #plt.plot(x, ACFoff,'k',alpha=0.3)
        ...
        if args.oned is None:
            ...
            if args.show_acf_plots:
                #2203//Commented out the functionality with the Are you happy part
#                fit=False
#                while fit==False:
                fig,ax=plt.subplots(2)
                ...
                ax[1].set_xlabel('Freq lag [MHz]')
                ax[1].set_ylabel('ACF freq')

                #Save command
                plot_name=pre_extension+"_ACF"+f"_comp{component}"+".pdf"
...
#                        fit=True
            ...
            if args.show_acf_plots:
                ...
                #Save command
                gs.tight_layout(fig)
                plot_name=pre_extension+"_2D_ACF"+f"_comp{component}"+".pdf"
                print(f"saved {plot_name}")
                plt.savefig(plot_name, facecolor='white', transparent=False)
                plt.show()
        ...
        #quantile plotting // nanpercentile to account for the masking
        #quantiles = np.nanpercentile(archive_orig, (1.0, 99.99)) 
        vmin_q = archive_orig.std() * -1
        vmax_q = archive_orig.std() * 7
        dyn = plt.subplot(gs[1], sharex=prof)
        dyn.imshow(archive_orig, aspect='auto', interpolation='Gaussian',
                   origin='lower', vmin=vmin_q, vmax=vmax_q)
        ...
        #Save command
        plot_name=pre_extension+"_edges_fluence"+f"_comp{component}"+".pdf"
        print(f"saved {plot_name}")
        plt.savefig(plot_name, facecolor='white', transparent=False)
        gs.tight_layout(fig)
        plt.show()
    ...
    if args.db is not None:
        data.to_csv(args.db, sep=",", header=True, index=True, na_rep='NA')
